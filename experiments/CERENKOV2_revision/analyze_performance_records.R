...
##               suffix)
##
## Requires    : reshape2, ggplot2, dplyr, arragements
##
...
make_results_plot <- function(p_measure_name, p_results_df) {
	...
	# trim out white spaces in `classifier_set_names`
    classifier_set_names <- gsub(" ", "", classifier_set_names)
    ...
    font_size = 9               
    p <- ggplot(df_plot_melted, aes_string(x="classifier_set_name", y=toupper(p_measure_name))) +
        geom_point(size=2) +
    ...
    ggsave(output_file_name, plot=p, width=1.25, height=2)
    ggsave(gsub(".pdf", ".png", output_file_name), plot=p, width=1.25, height=2)
}                             
...
get_perf_df_list <- function(hyperparameter_set_type_name) {
	ml_perf_df <- get(hyperparameter_set_type_name, g_ml_performance_results)
	ml_perf_df <- ml_perf_df[, c("test_auroc", "test_aupvr", "test_avgrank",
								 "classifier_set_name", "replication_id", "cv_fold_id")]

	clf_perf_df_list <- split(ml_perf_df, ml_perf_df$classifier_set_name)
	clf_perf_df_list <- lapply(clf_perf_df_list, function(df) {
		arrange(df, replication_id, cv_fold_id)
	})

	return(clf_perf_df_list)
}
...
pw_ttest <- function(clf_x_name, clf_y_name) {
	...
	for (metric in c("test_auroc", "test_aupvr", "test_avgrank")) {
		print(paste("metric:", metric))
		print(paste("classifier_set_name:", toString(paste(clf_x_name, "vs", clf_y_name))))
		
		x <- perf_df_lists[[clf_x_name]][[metric]]
		y <- perf_df_lists[[clf_y_name]][[metric]]
		
		print(paste("classifier_set_name in t-test: x vs y"))
		
		t_test_result <- t.test(x, y, paired=TRUE)
		print(t_test_result)
		
		pw_ttest_pvalues <- rbind(pw_ttest_pvalues, c(clf_x_name,
													  clf_y_name,
													  metric,
													  t_test_result$p.value))
	}
	return(pw_ttest_pvalues)
}

pw_ttest_pvalues <- apply(clf_pairs, MARGIN=1, FUN=function(row){pw_ttest(row[1], row[2])})
pw_ttest_pvalues <- do.call(rbind, pw_ttest_pvalues)

output_pvalue_file_name <- paste(g_output_file_base_name, "_pvalues.txt", sep="")
print(sprintf("Output pvalue file name: %s", output_pvalue_file_name))
write.table(pw_ttest_pvalues,
			file=output_pvalue_file_name,
			sep="\t",
			row.names=FALSE,
			col.names=FALSE,
			quote=FALSE)