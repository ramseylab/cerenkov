...
## ============================== load GWAVA feature data ==============================

print("loading GWAVA data")
g_gwava_feat_df <- readRDS(file="osu18_features_gwava(anonymous).rds") 
stopifnot(g_feature_matrix_is_OK(g_gwava_feat_df))
stopifnot(rownames(g_geom_feat_df) == rownames(g_annot_feat_df))
stopifnot(g_geom_feat_df$label == g_label_vec)

# Remove "label" column from dataframe
g_gwava_feat_df <- g_gwava_feat_df[, which(names(g_gwava_feat_df) != "label")]
...
g_gwava_published_hp_grid <- g_make_hyperparameter_grid_list(list(probability=c(FALSE),
                                                                  mtry=c(14),
                                                                  num.trees=c(100),  
                                                                  weight_positive_class=c(1),
                                                                  replace=c(TRUE),
                                                                  sample.fraction=c(1)))

g_classifier_list_gwava_published <- lapply(g_gwava_published_hp_grid,
                                            function(hp_cell) {
                                                list(classifier_feature_matrix_name="gwava_feat",
                                                     classifier_function_name="ranger",
                                                     classifier_hyperparameter_set_type_name="g_gwava_published_hp_grid",
                                                     classifier_set_name="GWAVA", 
                                                     classifier_hyperparameter_list=hp_cell)
                                            })
