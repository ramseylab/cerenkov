...
# ----- Pairwise t-tests ----- #
pw_ttest_pvalues <- c()
for (hyperparameter_set_type_names in names(g_ml_performance_results)) {
	print(paste("hyperparameter_set_type_names:", hyperparameter_set_type_names))
	
	ml_perf_df <- get(hyperparameter_set_type_names, g_ml_performance_results)[, c("test_auroc", "test_aupvr", "test_avgrank", 
	...
	for (metric in c("test_auroc", "test_aupvr", "test_avgrank")) {
		...
		metric_list <- lapply(clf_perf_df_list, "[[", metric)
		
		print(paste("classifier_set_name:", toString(names(metric_list))))
		stopifnot(length(names(clf_perf_df_list)) == 2)
		
		x <- metric_list[[1]]
		y <- metric_list[[2]]
		print(paste("classifier_set_name in t-test: x, y"))
		
		t_test_result <- t.test(x, y, paired=TRUE)
		print(t_test_result)
		
		pw_ttest_pvalues <- rbind(pw_ttest_pvalues, c(hyperparameter_set_type_names, 
													  names(metric_list)[1], 
													  names(metric_list)[2],
													  metric, 
