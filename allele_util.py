...
class AlleleUtil(AbstractFeatureUtil):
    ...
    @staticmethod
    def identify_major_vs_minor(snp_dfm, maf_threshold=0.05, remove_acgt_cols=False, revise_when_freqs_equal=False,
                                verbose=False):
        def identify(freq_series):
            ...
            """
            It is possible that `minor_freq` == `major_freq` == 0.5
                when `idxmin()` and `idxmax()` would pick the same allele.
            Therefore remove the `minor_allele` first then call `idxmax()`
            """
            major_allele = valid_freqs.drop(minor_allele).idxmax()
        ...
        mm_dfm = snp_dfm.loc[:, list('ATCG')].apply(lambda x: identify(x), axis=1)
        #I TOOK OUT THE 'reduce=False' argument because I got an error saying that it was not needed.
        # mm_dfm = snp_dfm.loc[:, list('ATCG')].apply(lambda x: identify(x), axis=1, reduce=False)
        snp_dfm = pd.concat([snp_dfm, mm_dfm], axis=1)
        ...
        if revise_when_freqs_equal:
            ...
            if verbose:
                if any(has_equal_freq):
                    ...
                    print("[identify_major_vs_minor] No alleles with equal freqs detected")
                ...
                if any(idx):
                    ...
                    print("[identify_major_vs_minor] No entries to swap alleles")
    ...
    def flip_alleles_on_rev_strand(snp_dfm, remove_strand_col=False, verbose=False):
        ...
        if remove_strand_col:
            if verbose:
                print("[flip_alleles_on_rev_strand] going to drop column 'strand'")
