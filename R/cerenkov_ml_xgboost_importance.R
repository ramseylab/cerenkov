...
## cerenkov_ml_xgboost_importance.R -- obtaines feature importance scores for the OSU feature matrix
## on the set of 15,331 ground-truth SNPs, using xgboost.
##
...
g_par <- list(
    num_folds_cross_validation = 1,   
    num_cv_replications = 1,          
    flag_create_fork_cluster = FALSE, 
    override_num_fork_processes = 50, 
    show_progress_bar = FALSE,
    flag_locus_sampling = TRUE,        ## set to false if you want SNP-level sampling
    flag_xgb_importance = TRUE,        ## if you set this to TRUE, make sure you set num_cv_replications=1 and num_folds_cross_validation=1
...
g_hyperparameter_grid_list_xgb <- g_hyperparameter_grid_list_xgb_best_aupvr
...
if (! is.null(g_ml_results)) {
    save("g_par",
         ...
         file="cerenkov_ml_xgboost_importance.Rdata")
}
...
write.table(g_ml_results$feature_impt_scores$XGB[[1]][[1]],
            file="cerenkov_xgboost_feature_importance_scores.txt",
            sep="\t",
            row.names=FALSE,
            col.names=TRUE,
            quote=FALSE)
